...
const LoadingComponent = ({ classes, history }) => {
	...
	useEffect(() => {
		const tick = () => {
			...
			const newProgress = Math.min(100, progress + Math.floor(Math.random() * 40) + 1);
			if (newProgress === 100) {
				setProgress(100)
			...
			console.log(`Progressing to ${newProgress} , Loading will take${time}ms`)
			setTimeout(() => {
	...
	return (
		<div>
			...
			<div className={classes.progressContainer}>
				...
				<div className={classes.progressPhrase}>
					{phrase}
				</div>
			</div>
